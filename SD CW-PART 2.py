...
def progression_list(progression_outcome,pcredits,dcredits,fcredits):
    '''Adds the progression data and credits of each student to a list and returns the list'''
...
while True:
    ...
    if total !=120:
        ...
        progress+=1
        finallist.append(progression_list('Progress -',passcredits,defercredits,failcredits))
       

    elif (passcredits==100):
        print ("Outcome: Progress (Module Trailer)")
        trailer+=1
        finallist.append(progression_list('Progress(Module Trailer)-',passcredits,defercredits,failcredits))
        
    elif (failcredits>=80):
        print ("Outcome: Exclude")
        exclude+=1
        finallist.append(progression_list('Exclude -',passcredits,defercredits,failcredits))
        
    else:
        print ("Outcome: Do not progress (Module Retriever)")
        retriver+=1
        finallist.append(progression_list('Module Retriever -',passcredits,defercredits,failcredits))
