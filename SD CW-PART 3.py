...
def list_and_textfile(progression_outcome,pcredits,dcredits,fcredits):
    '''Takes the progression outcome and credit values, stores them in a variable(t) as a string and writes it to a text file.
    Then adds the same data to a list and returns the list  '''
    
    t=progression_outcome+str(pcredits)+', '+str(dcredits)+', '+str(fcredits)+"\n"   #turns the credit scores to string, concatinates them to a single string and assigns to variable t
    
    textfile.write(t)
    
    progress_list=[progression_outcome,pcredits,dcredits,fcredits]
...
textfile=open("Progression_data.txt","w")
textfile.close()

textfile=open("Progression_data.txt","a")
textfile.write("Part 3"+'\n')
...
while True:
    ...
    if total !=120:
        ...
        progress+=1
        finallist.append(list_and_textfile('Progress - ',passcredits,defercredits,failcredits)) #the list finallist is appended with the return value of list_and_textfile function
      
    elif (passcredits==100):
        print ("Outcome: Progress (Module Trailer)")
        trailer+=1
        finallist.append(list_and_textfile('Progress(Module Trailer)- ',passcredits,defercredits,failcredits))


    elif (failcredits>=80):
        print ("Outcome: Exclude")
        exclude+=1
        finallist.append(list_and_textfile('Exclude - ',passcredits,defercredits,failcredits))
        
    else:
        print ("Outcome: Do not progress (Module Retriever)")
        retriver+=1
        finallist.append(list_and_textfile('Module Retriever - ',passcredits,defercredits,failcredits))
